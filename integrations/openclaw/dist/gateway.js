 ...
 */
export async function napStartupHook(config, currentEndpoint, opts = {}) {
    if (!config.enabled)
        return null;
    ...
    if (state.user_token && currentEndpoint) {
        try {
            ...
            log(`[NAP] Warning: could not sync endpoint — ${err.message}`);
        }
    }
    return state;
}
 ...
 */
export function createAgentCardHandler(state) {
    return (req, res) => {
 ...
 */
export function startNAPSync(config, getEndpoint, onStateUpdate, opts = {}) {
    const log = opts.log ?? console.log;
    ...
    const timer = setInterval(async () => {
        try {
            ...
            log(`[NAP] Sync error: ${err.message}`);
        }
    }, intervalMs);
...
// ── Internal helpers ──────────────────────────────────────────────────────────
async function syncEndpoint(config, state, endpoint, log) {
    if (!state.user_token)
        return;

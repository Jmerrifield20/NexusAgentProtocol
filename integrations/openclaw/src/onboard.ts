 ...
 */
export async function onboardWizard(
  config: Partial<NAPConfig>,
  ...
  try {
    ...
    let registrationType: 'hosted' | 'domain' = 'hosted';
    ...
    const descriptionRaw =
      config.description ?? (await rl.question('Short description (optional): '));
    const description = descriptionRaw || undefined;

    const endpointRaw =
      config.endpoint ??
      (await rl.question(
        'Public endpoint URL (e.g. https://myagent.example.com) — leave blank to set later: ',
      ));
    ...
    if (config.registry_url) napConfig.registry_url = config.registry_url;
    if (description) napConfig.description = description;
    if (endpoint) napConfig.endpoint = endpoint;
 ...
 */
export async function activateWizard(
  config: NAPConfig,
  opts: { log?: (msg: string) => void } = {},
): Promise<void> {
  const log = opts.log ?? console.log;
...
async function readPassword(
  ...
  const tty = stdout.isTTY ? (stdout as NodeJS.WriteStream & { isTTY: boolean }) : null;
  if (tty && process.stdin.isTTY) {
    (process.stdin as NodeJS.ReadStream & { setRawMode?: (raw: boolean) => void }).setRawMode?.(true);
  }

  const pass = await rl.question('');

  if (tty && process.stdin.isTTY) {
    (process.stdin as NodeJS.ReadStream & { setRawMode?: (raw: boolean) => void }).setRawMode?.(false);
  }

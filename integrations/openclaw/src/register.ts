...
async function registerHosted(
  ...
  const reg = await client.registerHosted(
    token,
    config.display_name,
    endpoint,
    config.description,
...
async function registerDomainVerified(
  client: NAPClient,
  config: NAPConfig,
  endpoint: string,
  log: (msg: string) => void,
  ...
  const reg = await client.registerDomain(
    domain,
    capability,
    config.display_name,
    endpoint,
...
export async function activateAgent(
  ...
  if (token && !isTokenFresh(state)) {
    ...
    throw new Error(
      'NAP user token has expired. Re-authenticate with: openclaw nap login',
    );
  }
  ...
  const updated: NAPState = { ...state, status: 'active' };
  if (result.agent_card_json) updated.agent_card_json = result.agent_card_json;
  if (result.task_token) updated.task_token = result.task_token;
